...
|
| 用户地址列表 | /addresses?user_id={user_id} | GET | 用户地址列表 |
| 添加用户地址 | /addresses?user_id={user_id} | POST | 增加一条用户地址信息 |
| 清空用户地址信息 | /addresses?user_id={user_id} | DELETE | 清空用户地址信息 |
...
|
| 获取订单列表 | /orders?type={all,unpay,unship,unrecv,unrate}&user_id={user_id} | GET | 获取用户订单列表 |
| 清空订单 | /orders?user_id={user_id} | DELETE | 清空订单 |
| 添加订单信息 | /orders?user_id={user_id} | POST | 新增订单 |
...
| 获取用户道具列表| /props?user_id={user_id} | GET | 获取用户所有道具 |
...
- 接口地址: /carts/{cart_id}/{goods_id}
- 请求方法: POST
- 请求参数: cart_id->购物车ID goods_id->产品ID
- 接口说明: 将指定产品加入购物车
...
- 接口地址: /carts/{cart_id}/{order_item_id}
- 请求方法: DELETE
